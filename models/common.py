...
class DetectMultiBackend(nn.Module):
    ...
    def forward(self, im, im_d, im_t, augment=False, visualize=False):
        ...
        if self.pt:  # PyTorch
            y = self.model(im, im_d, im_t, augment=augment, visualize=visualize) if augment or visualize else self.model(im, im_d, im_t)
    ...
    def warmup(self, imgsz=(1, 3, 640, 640)):
        ...
        if any(warmup_types) and (self.device.type != "cpu" or self.triton):
            ...
            for _ in range(2 if self.jit else 1):  #
                self.forward(im, im_d, im_t)  # warmup
