...
def train(hyp, opt, device, callbacks):
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        optimizer.zero_grad()
        for i, (imgs, imgs_d, imgs_inf, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            callbacks.run("on_train_batch_start")
            ni = i + nb * epoch  # number integrated batches (since train start)
            imgs = imgs.to(device, non_blocking=True).float() / 255  # uint8 to float32, 0-255 to 0.0-1.0
            imgs_d = imgs_d.to(device, non_blocking=True).float() / 255
            imgs_inf = imgs_inf.to(device, non_blocking=True).float() / 255
            ...
            if opt.multi_scale:
                ...
                if sf != 1:
                    ...
                    imgs_d = nn.functional.interpolate(imgs_d, size=ns, mode="bilinear", align_corners=False)
                    imgs_inf = nn.functional.interpolate(imgs_inf, size=ns, mode="bilinear", align_corners=False)

            # Forward
            with torch.cuda.amp.autocast(amp):
                pred = model(imgs, imgs_d, imgs_inf)  # forward
...
def parse_opt(known=False):
    ...
    parser.add_argument("--epochs", type=int, default=99, help="total training epochs")
