...
def augment_hsv(im, img_d, img_inf, hgain=0.5, sgain=0.5, vgain=0.5):
...
def random_perspective(
    im, im_d, im_inf, targets=(), segments=(), degrees=10, translate=0.1, scale=0.1, shear=10, perspective=0.0, border=(0, 0)
):
    ...
    im_d_norm = cv2.normalize(im_d, None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX)
    im_d_norm = np.asarray(im_d_norm, dtype=np.uint8)
    im_d = cv2.applyColorMap(im_d_norm, cv2.COLORMAP_JET)

    # colormap = Image.fromarray(colormap)
    # colormap.save("/data/Disk_D/yangxiao/Project/ICPR/yolov5-master/utils/123.png")
    ...
    if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
        if perspective:
            ...
            im_d = cv2.warpPerspective(im, M, dsize=(width, height), borderValue=(114, 114, 114))
            im_inf = cv2.warpPerspective(im, M, dsize=(width, height), borderValue=(114, 114, 114))
        else:  # affine
            im = cv2.warpAffine(im, M[:2], dsize=(width, height), borderValue=(114, 114, 114))
            im_d = cv2.warpAffine(im, M[:2], dsize=(width, height), borderValue=(114, 114, 114))
            im_inf = cv2.warpAffine(im, M[:2], dsize=(width, height), borderValue=(114, 114, 114))
    ...
    return im, im_d, im_inf, targets


def copy_paste(im, im_d, im_inf, labels, segments, p=0.5):
    """
    ...
    if p and n:
        ...
        im_d_new = np.zeros(im_d.shape, np.uint8)
        im_inf_new = np.zeros(im_inf.shape, np.uint8)
        for j in random.sample(range(n), k=round(p * n)):
            ...
            if (ioa < 0.30).all():  # allow 30% obscuration of existing labels
                ...
                cv2.drawContours(im_d_new, [segments[j].astype(np.int32)], -1, (1, 1, 1), cv2.FILLED)
                cv2.drawContours(im_inf_new, [segments[j].astype(np.int32)], -1, (1, 1, 1), cv2.FILLED)

        result = cv2.flip(im, 1)  # augment segments (flip left-right)
        i = cv2.flip(im_new, 1).astype(bool)
        im[i] = result[i]  # cv2.imwrite('debug.jpg', im)  # debug

        result_d = cv2.flip(im_d, 1)
        i_d = cv2.flip(im_d_new, 1).astype(bool)
        im_d[i_d] = result_d[i_d]

        result_inf = cv2.flip(im_inf, 1)
        i_inf = cv2.flip(im_inf_new, 1).astype(bool)
        im_inf[i_inf] = result_inf[i_inf]
    return im, im_d, im_inf, labels, segments
