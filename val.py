...
def run(
    ...
    for batch_i, (im, im_d, im_inf, targets, paths, shapes) in enumerate(pbar):
        callbacks.run("on_val_batch_start")
        with dt[0]:
            if cuda:
                im = im.to(device, non_blocking=True)
                im_d = im_d.to(device, non_blocking=True)
                im_inf = im_inf.to(device, non_blocking=True)
                targets = targets.to(device)
            im = im.half() if half else im.float()  # uint8 to fp16/32
            im /= 255  # 0 - 255 to 0.0 - 1.0
            nb, _, height, width = im.shape  # batch size, channels, height, width

            im_d = im_d.half() if half else im_d.float()
            im_d /= 255
            im_inf = im_inf.half() if half else im_inf.float()
            im_inf /= 255
        ...
        # Inference
        with dt[1]:
            preds, train_out = model(im, im_d, im_inf) if compute_loss else (model(im, im_d, im_inf, augment=augment), None)
...
def parse_opt():
    ...
    parser.add_argument("--weights", nargs="+", type=str, default=ROOT / "/data/Disk_D/yangxiao/Project/ICPR/yolov5-master/runs/train/exp2/weights/best.pt", help="model path(s)")
